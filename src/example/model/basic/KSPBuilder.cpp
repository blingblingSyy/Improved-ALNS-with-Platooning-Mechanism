    ...
    while(KShortestPath_Start_End.size() < k_limit && !KDeviationPath_Start_End.empty())
    {
        ...
        for(int i = 0; i < last_SP.size()-1; i++) //for every deviation node in the shortest path except the end_node
        {
            ...
            if(!subtour) //if no subtour
            {
                ...
                bool appear_in_A = find(KShortestPath_Start_End.begin(), KShortestPath_Start_End.end(), tempSP_Start_End) != KShortestPath_Start_End.end();
                bool appear_in_B = find(KDeviationPath_Start_End.begin(), KDeviationPath_Start_End.end(), tempSP_Start_End) != KDeviationPath_Start_End.end();
